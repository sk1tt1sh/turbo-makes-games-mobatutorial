...
public class ClientConnectionManager : MonoBehaviour {
  ...
  private void StartServer() {
    World serverWorld = ClientServerBootstrap.CreateServerWorld("TurboServerWorld");
    NetworkEndpoint serverEndpoint = NetworkEndpoint.LoopbackIpv4.WithPort(Port);
    {
  ...
  private void StartClient() {
    World clientWorld = ClientServerBootstrap.CreateClientWorld("ChampionWorld");
    World.DefaultGameObjectInjectionWorld = clientWorld;

    NetworkEndpoint clientConnection = NetworkEndpoint.Parse(Address, Port);
    {
      using var networkDriverQuery = clientWorld.EntityManager.CreateEntityQuery(ComponentType.ReadWrite<NetworkStreamDriver>());
      networkDriverQuery.GetSingletonRW<NetworkStreamDriver>().ValueRW.Connect(clientWorld.EntityManager, clientConnection);
    }
